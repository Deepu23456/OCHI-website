...
export const Navbar = ({ toggleNav, isOpen }) => {
  ...
  return (
    <>
      <div
        className={`fixed backdrop-blur-lg  z-[999] transition-all font-[Neue_Montreal] w-full max-md:px-6 max-sm:px-4 px-16 py-8 flex justify-between items-center ${
      ...
      >
        ...
        <div className="menu text-[3vw] hidden max-xl:flex max-xl:text-[4vw] max-md:text-[6.5vw] max-sm:text-[9.2vw]">
          {isOpen ? (
            <RiCloseLargeLine onClick={toggleNav} />
          ) : (
            <CgMenuRightAlt onClick={toggleNav} />
          )}
        </div>
        <div className="links flex gap-10 max-xl:hidden">
          {["Services", "Our work", "About us", "Insights", "Contact"].map(
            (item, index) => {
              return (
                <a
                  key={index}
                  className={`font-light text-[1.1vw] ${index == 4 && "ml-36"}`}
                >
                  {item}
                </a>
              );
